...
import com.example.moattravel_2.form.ReservationInputForm;
import com.example.moattravel_2.form.ReservationRegisterForm;
...
import com.example.moattravel_2.security.UserDetailsImpl;
import com.example.moattravel_2.service.ReservationService;
import com.example.moattravel_2.service.StripeService;
...
public class ReservationController {
	...
	public String index(@AuthenticationPrincipal UserDetailsImpl userDetailsImpl, @PageableDefault(page = 0, size = 10, sort = "id", direction = Direction.ASC) Pageable pageable, Model model) {
		//@AuthenticationPrincipalは現在ログインしているユーザー情報を取得する
						...
						Model model)
	{
		...
		Integer capasity = house.getCapacity();
		
		if(numberOfPeople != null) {
			if(!reservationService.isWithinCapacity(numberOfPeople, capasity)) {
				//falseが返ってきたら
		...
		//addFlashAttributeはリダイレクト後の1回だけ使える一時的なデータ保持
