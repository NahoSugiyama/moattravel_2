...
public class UserController {
	...
	public String index(@AuthenticationPrincipal UserDetailsImpl userDetailsImpl, Model model) {
		...
		/*  @AuthenticationPrincipalでログイン中のユーザー情報を取得可能にする
			アノテーションをつける引数はUserDetailsインターフェースを実装したクラス
		*/
		
		User user = userRepository.getReferenceById(userDetailsImpl.getUser().getId());
		model.addAttribute("user", user);//htmlの"user"変数にuserを渡している
	...
	public String update(@ModelAttribute @Validated UserEditForm userEditForm, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
		...
		if (userService.isEmailCahanged(userEditForm) && userService.isEmailRegistered(userEditForm.getEmail())) {
